 ...
 * Evaluator Store
 *
 * Manages evaluation agent sessions using Svelte 5 runes.
 */
...
const state = $state<EvaluatorState>({
	sessions: [],
	currentSession: null,
	isEvaluating: false,
	streamingOutput: '',
...
export const evaluatorStore = {
	...
	// Actions
	createSession(request: EvaluationRequest): EvaluationSession {
	...
	completeSession(sessionId: string, assessment: EvaluationAssessment): void {
		const session = state.sessions.find((s) => s.id === sessionId);
		if (!session) return;
