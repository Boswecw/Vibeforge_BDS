    ...
    if (!value || typeof value !== "object") {
      ...
      if (typeof value[0] === "string") {
        ...
        if (reviver) {
          ...
          return hydrated[index] = reviver(hydrate2(i));
        }
        switch (type) {
          case "Date":
        ...
        if (others.length > 0) {
          ...
          if (queued_root_effects.length > 0) {
            ...
            for (const root2 of queued_root_effects) {
              batch2.#traverse_effect_tree(root2, dummy_target);
            }
            batch2.deactivate();
          }
  ...
  if (page_config.ssr) {
    ...
    try {
      ...
      rendered = await with_request_store({ event, state: event_state }, async () => {
        ...
        const { head: head3, html: html2, css } = options2.async ? await rendered2 : rendered2;
        return { head: head3, html: html2, css };
      });
    } finally {
      reset();
    }
...
class Renderer {
  ...
  option(attrs, body2, css_hash, classes, styles, flags2) {
    ...
    const close = (renderer, value, { head: head2, body: body3 }) => {
      ...
      renderer.#out.push(`>${body3}</option>`);
      if (head2) {
...
function query(validate_or_fn, maybe_fn) {
  ...
  const wrapper = (arg) => {
    ...
    const { event, state: state2 } = get_request_store();
    const get_remote_function_result = () => run_remote_function(event, state2, false, arg, validate, fn);
...
export {
  ...
  set_manifest as v,
  writable as w,
  set_prerendering as x,
  set_private_env as y,
  set_public_env as z
};
